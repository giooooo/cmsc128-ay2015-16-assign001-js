...
var converter = {
    ...
    wordsToCurrency : function (input, currency){
        switch(currency){
            case 'USD' : return "USD" + converter.wordsToNumber(input);
            case 'JPY' : return "JPY" + converter.wordsToNumber(input);  
            case 'PHP' : return "PHP" + converter.wordsToNumber(input);
        }
    },

    wordsToNumber : function (input) {
        var out = 0,
            put = 0;
        input = input.split(/[\s-]/);
        ...
        if((input.indexOf('thousand') == -1)){
            ...
            out = converter.wordToNumberByThrees(input.splice(0, input.indexOf('thousand')).join(' ')) * 1000;
            put = converter.wordToNumberByThrees(input.splice(input.indexOf('thousand')).join(' '));        
        }

        return out+put;
    },

    numberDelimited : function (number, delimiter, jumps) {
    ...
    // helpers
    wordToNumberByThrees : function (input){
        input = input.split(/[\s]/);
        var output = 0;
